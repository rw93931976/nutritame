...
async def create_coach_session(session_request: CoachSessionCreate):
    ...
    try:
        ...
        if not disclaimer_accepted and (user_id.startswith('demo-') or 'demo' in user_id.lower()):
            try:
                logging.info(f"Auto-creating demo consent for user: {user_id}")
                consented_at_utc = datetime.now(timezone.utc)
                consent_id = str(uuid.uuid4())
                
                # Generate demo consent record
                signature = generate_consent_signature(
                ...
                logging.info(f"Demo consent auto-created for user: {user_id}")
                
            except Exception as e:
                logging.error(f"Failed to auto-create demo consent: {e}")
